...
- (BOOL)loadShaders {
  ...
  GLint aPosLoc = glGetAttribLocation([_prog program], "aPos");
  glEnableVertexAttribArray(aPosLoc);
  
  GLint aTexLoc = glGetAttribLocation([_prog program], "aTexCoord");
  ...
  for (int i = 0; i < SizeY; i++) {
    for (int j = 0; j < SizeX; j++) {
      if (j < (SizeX / 2.0)) {
        uint8_t random[4] = { 255, 0, 0, 255 };
        [pixels appendBytes:random length:4];
      } else {
        uint8_t random[4] = { 0, 0, 255, 255 };
        [pixels appendBytes:random length:4];
      }
    }
  }
  ...
  [self createAndBindTexture:&_textureMain2N pixels:pixels scale:1.0f fbo:_frameBuffers[1] filter:GL_NEAREST];
  [self createAndBindTexture:&_textureMainL pixels:pixels scale:1.0f fbo:_frameBuffers[0] filter:GL_LINEAR];
  [self createAndBindTexture:&_textureMain2L pixels:pixels scale:1.0f fbo:_frameBuffers[1] filter:GL_LINEAR];
...
- (void)advance {
  ...
  [self setUniforms:[_prog program]];
  if (_it > 0) {
    glActiveTexture(GL_TEXTURE0);
